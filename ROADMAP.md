...
**Current version**: 0.333.0 (2026-08-23) — 71,900+ lines of Zig (47,000+ production + 24,800+ tests across 331 `.zig` files), 2,100 unit tests, 85 lint rules, 112 `REVERSE_MAP` entries, 43 golden inputs (36 runner-covered suites), 12 generators × 6 dialects.
...
- [ ] Windows-native `test_migrate_status.sh` null-byte failures (5/7 suites fail through bash pipes; output encoding, pre-existing since before v0.332.0). — `a_id INT REFERENCES users(id) ON DELETE CASCADE` (the dominant mysqldump/SQLite .schema style) was silently dropped by the SQL parser and its action fragments leaked in as ghost fields; now captured on the column and emitted alongside table-level constraints with correct bare-reference/default-action handling (reverse suite 21 → 24). — resource audit: LSP compiled every recompile onto the process arena (thousands of didChange → unbounded growth of pipeline intermediates); now each document owns an arena destroyed on recompile/close (session memory is O(open docs) not O(total requests)). Watch mode skipped no work when nothing changed; mtime short-circuit cuts steady-state polls from N×read+hash to N×stat. — the `# name { ... }` form was a ghost syntax: LSP folding/format/tune all supported it (v0.329.0 fixed two of them under the false belief the parser did too) while the parser turned `# users {` into a table named `{`, swallowed `}` as a varchar field, and silently emitted broken SQL for single-table schemas. Parser now strips header braces and flushes blocks on `}` lines; documented in schemaspec §1 + grammar.ebnf; first table-level brace goldens added. Also fixed three latent leaks found by the new parser tests (printDiagnostic per-call leak, BlockState.deinit missing embeds, parse() early-return skipping cleanup). — `rune format` had no branch for the table-closing `}` (indented it to `  }`, never reset block state, and was not idempotent: every brace-syntax schema got corrupted by `--write`); `rune tune` collected `}` as a field and kept `{` in table names. Both now handle braces structurally with a depth counter (`@if(...) {` opens a nested block); formatter golden set gained the first-ever brace-syntax cases (all six previous goldens used legacy brace-less syntax, which is why this escaped); format goldens converted from CRLF (failing everywhere since v0.250.0 while CI passed); formatter suite added to the coverage runner (#36). — deep analysis surfaced a cluster of "same fact stated in N places" defects, all fixed: `lint --show-rules`/`--init` were unreachable (flag-gate + dispatch drops), per-command help rendered the wrong command's options (position-coupled arrays had drifted — now name-keyed with comptime coverage enforcement), lint rule count in help was hardcoded 84 vs actual 85 (now comptime-derived), `rune stats` text went to stderr while JSON went to stdout (unified on stdout via writer variants), bench gate false-failed from Debug trees (bench now pinned ReleaseSafe), cache.zig's disk-load parser never worked plus two memory bugs and handleShare leaks (found by newly activated tests), and two golden suites hardcoded the ELF path (broken on Windows). Unit tests 2068 → 2077.
