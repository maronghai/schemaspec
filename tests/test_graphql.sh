#!/usr/bin/env bash
# ── Rune GraphQL Test Runner ──
# Tests: rune generate graphql produces expected GraphQL SDL output.
...
for ss_file in "$TEST_DIR"/graphql-*.ss; do
  [ -f "$ss_file" ] || continue
  ...
  if ! "$COMPILER" generate graphql "$ss_file" -o "$tmp_file" 2>/dev/null; then
