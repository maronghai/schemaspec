...
./rune/zig-out/bin/rune lint schema.ss                    # Lint schema for quality issues (39 rules)
...
| | `lint.zig` | Lint barrel — re-exports from `lint/rules.zig` (39 rules + `RuleInfo` + `RULE_INFO`), `lint/handlers/*.zig` (8 handler modules), `lint/format.zig` (text/JSON/SARIF), `lint/config.zig` (LintConfig, LintRule enum with `name()`, `description()`, `isFixable()`, TOML parsing), `lint/fix.zig` (auto-fix for 10 rules) |
| | `cli/lint_cmd.zig` | Lint CLI handler (extracted from main.zig) |
