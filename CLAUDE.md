...
./rune/zig-out/bin/rune lint schema.ss                    # Lint schema for quality issues (86 rules)
...
rune/src/
  ...
  tests.zig                                                       # colocated test index (119 files)
...
- **Brace Syntax Is Official (v0.330.0)** (`parser/parse_table.zig`, `parser/parser.zig`): `# name { ... }` is a first-class table form — parseTableHeader strips a trailing `{` token from the header (has_brace is informational), and a lone `}` line flushes the current table/template/composite block (a stray `}` outside any block warns). Field names can never be `}`, so the check is unambiguous; CHECK-constraint braces (`{a,b}`) live inside field token streams and never conflict. Documented in schemaspec/schema.md §1 and grammar.ebnf table_decl. History: LSP folding_range/format/tune supported this shape before the parser did — v0.329.0's "main pipeline natively supports it" claim was wrong until this release.
- **Formatter Brace-State Invariant** (`formatter.zig`): `formatDialect` tracks brace depth — `# name {` opens a block (indented body), a lone `}` closes it at column 0 and resets block state; `@if(dialect=x) {` opens a nested block whose `}` is indented like a field. Formatting is idempotent: golden tests assert format(format(x)) == format(x). Source-rewriting tools must treat `{`/`}` as structural (tune.zig's parseTableBlocks excludes them from fields) — LSP folding_range is the reference implementation of the stack discipline.
...
- **LSP Per-Document Arena (v0.331.0)** (`lsp/server.zig`): each document's compilation runs in a dedicated `ArenaAllocator` (`doc_arenas`); every recompile destroys the previous arena and compiles fresh — pipeline intermediates die with the document version instead of accumulating in the process arena across an editing session. Diagnostics are duped into the long-lived server arena; feature handlers read TypedAst from the doc-scoped arena whose lifetime is exactly the document's current version.
- **LSP Pipeline Parity Invariant (v0.334.0)** (`lsp/compile_service.zig`, `diagnostic.zig`): the LSP compile service must run the exact same stages as the CLI pipeline — splitLines before tokenize (the tokenizer is line-oriented; feeding the whole document as one slice silently zeroes every AST-based feature), and per-error diagnostics captured via the `active_collector` routing hook in diagnostic.zig (the parser prints some diagnostics directly to stderr; the hook redirects them into a DiagnosticCollector so editors get each error's real location). CLI paths leave the hook null and re-emit the captured output for unchanged stderr behavior. References/highlights locate FK names on `fk.line_no` (not the table header line); rename is AST-driven — edits only at AST-known positions, never free-text scans (comments must not be renamed).
- **Compile-Mode Output Parity (v0.334.0)** (`codegen/streaming.zig`, `pipeline/handlers.zig`): sequential, `--stream`, and `--stream --parallel` must produce byte-identical SQL for the same schema — including the `-- Schema version:` comment and CREATE DATABASE statement (`formatStreamingResultWithHeader`). Cache-hit SQL aliases cache-owned memory: format streaming results BEFORE deinit-ing the table cache. Cycle detection in imports uses a recursion-path set (`in_progress`), not just the visited set — diamonds are legal, true cycles are not; child parse errors accumulate into the root tree's error_count so `--check` gates see them.
- **Table-Driven LSP Method Dispatch** (`lsp/server.zig`): LSP method routing uses a `DISPATCH_TABLE` array of `{method, handler}` entries. All 22 handlers share the unified `HandlerFn` signature `(self, stdout, id, params)`. Adding a new LSP method = add entry to `DISPATCH_TABLE` + add handler in `lsp/handlers.zig`. Eliminates the 22-branch if-else chain and 22 adapter functions that previously existed.

- **Data-Driven Lint Rule Dispatch** (`lint/rules.zig`): Lint rule execution uses a `RULES` array of `{rule, handler}` entries. Each handler receives the full AST + config and iterates over relevant entities (tables, custom types, or entire schema). Handlers are organized into category modules: `lint/handlers/structural.zig` (11 rules), `lint/handlers/naming.zig` (8 rules), `lint/handlers/validation.zig` (11 rules), `lint/handlers/compat.zig` (3 rules), `lint/handlers/fk.zig` (9 rules), `lint/handlers/index.zig` (5 rules), `lint/handlers/view.zig` (4 rules), `lint/handlers/enum.zig` (4 rules), `lint/handlers/portability.zig` (3 rules). Adding a new lint rule = add entry to `RULES` + implement handler function in appropriate module. Replaces 22 repetitive guard-then-call blocks with a single loop. **Modifier semantics invariant (v0.326.0)**: `.auto_inc`/`.auto_inc_pk` on datetime types means DEFAULT CURRENT_TIMESTAMP (parser whitelist allows `n, N, t, d`) — auto-increment lint rules must gate through `hasIdentityModifier` (`validation.zig`, excludes datetime), and default-value rules treat datetime+`+` as already-defaulted via `hasExplicitDefault`; the fixer's `detectDefaultValue` likewise skips such lines. **Fixer output syntax invariant (v0.333.0)**: appended defaults must use the spec's attached form with one separator space (` =0`, not ` = 0` — the tokenizer splits fields on whitespace, so a detached value is warned about then silently dropped), and every multi-line insertion must end with `\n` (a missing one fuses the next table header into a field line).
...
- **Conditional Schema Blocks** (`parser/parser.zig`, `semantic/pass/resolve_conditionals.zig`): `@if(dialect=pg|sqlite)` ... `@endif` blocks within table definitions allow dialect-specific fields. Fields inside the block are only included when compiling for a matching dialect. The parser records conditional blocks in `Table.conditional_blocks`, and the `resolve_conditionals` semantic pass filters them based on the target dialect. The pass runs after `resolve_names` and before `autofk`. Conditional-block field indices are recorded pre-template-merge; `applyTemplateWithOrigin`/`remapConditionalBlocks` in `semantic/template.zig` re-derive them after merging so ranges survive template field insertion (v0.321.0). `resolve_conditionals` also remaps `embeds.insert_pos` to the filtered field list (v0.325.0), and embed lines inside a block carry the block's dialects via `CompositeEmbed.dialects` (an embed adds no field, so ranges can't cover it — see Composite Types). The target dialect must reach every compile path — file-based compilation forwards it explicitly since v0.321.0.

- **Composite Types** (`parser/parser.zig`, `semantic/pass/resolve_composites.zig`, v0.320.0): reusable field groupings declared at top level (`* name` + field lines) and embedded inside table bodies (`*name`, no space). Embeds expand **in place** at their position in the field list, carrying all modifiers/defaults/CHECKs/comments/FKs. The parser records declarations in `Ast.composites` and embed sites (name + insert position + optional @if dialect gate) in `Table.embeds`; the `resolve_composites` semantic pass splices them before `autofk` runs (and after `resolve_conditionals`, so conditional-block indices are already consumed). **Invariant (v0.325.0)**: `insert_pos` refers to the literal (pre-template-merge, pre-filter) field list; every pass that adds/removes fields before expansion must remap it — `remapEmbeds` in `semantic/template.zig` (template merge, via the origin map) and `resolve_conditionals` (dialect filtering) both do. Embeds inside unmatched @if blocks are skipped without triggering unknown-composite errors or unused-composite warnings. Errors: unknown reference, duplicate definition, empty composite; unused composites warn. Composites vs templates: templates merge whole-table with slot control; composites embed field groups at any position, multiple times per table.
...
- **Inline Column-Level FK Reverse (v0.332.0)** (`parser/sql_parser_create.zig`, `reverse/codegen.zig`): `a_id INT REFERENCES users(id) ON DELETE CASCADE` is captured as `SqlColumn.inline_fk` (table, optional column list, full action chain) and emitted by `emitForeignKeys` alongside table-level constraints. classifyFk omits empty reference parentheses (bare `REFERENCES t` → `> (col) t`) and emits nothing for RESTRICT/NO ACTION — they are the .ss omission default; the language's only action tokens are `-C`/`-N`/`C`/`N`.
- **Reverse Pipeline Robustness Contract (v0.335.0)**: dialect auto-detection matches patterns with non-identifier word boundaries against a comment-blanked copy of the SQL (`dialect_detect.zig`) — a `-- RESTRICT` comment must never score SQLite's STRICT. SQL keyword comparison in `sql_parser*.zig` is ASCII case-insensitive (lowercase dumps are first-class); REVERSE_MAP exact-match folds case too, so cross-dialect passthrough entries at lower rev_priority can win over dialect-specific entries carrying the same spelling. The reverse JSON emitter manages separators through a per-object first-field flag — never unconditional trailing commas; SARIF message strings must keep all suffixes inside one escaped string. `ALTER TABLE t ADD [COLUMN] c …` parses through the same column path as CREATE TABLE.
- **Forward Parse Lenience Contract (v0.335.0)**: identifier-shaped default values absorb their tokenizer-split balanced call parens back into the token (`=NOW()` → `DEFAULT NOW()`, verbatim via `emitDefault`; numeric defaults followed by `(…)` remain CHECK ranges). A multi-char type identifier immediately followed by balanced parens is a raw_sql type (`CHAR(2)` survives recompilation instead of degrading to a fabricated CHECK). All file/stdin read entry points strip a leading UTF-8 BOM (`io.zig stripBom`). Backtick-quoted segments survive space splitting in the tokenizer; table/field names strip surrounding quotes; reverse emits backtick-quoted headers for any name containing non-identifier characters. Lint fixers never append into lines carrying inline comments, and EOF timestamp insertion only fires when the source ends inside a table body (`fix.zig`).
- **Flag Forwarding & Emitter Escaping Contract (v0.336.0)**: global flags with subcommand-specific namespaces (`--format`) are consumed AND forwarded (pass-through into filtered_args like `-o`); export/lint/docs re-parse them locally — never map one command's format enum onto another's. Every JSON emitter routes names/values/comments through `utils.jsonEscapeString` (or escapes the fully rendered string); SQL single-quoted literals double apostrophes via `dialect/common.writeSqlSingleQuoted`; Markdown cells escape pipes/newlines (`docs.writeMarkdownCell`) and Mermaid entity ids use quoted form for non-identifier names. The tokenizer keeps `'…'`/`"…"` segments as single tokens across spaces (same-quote-close required, unterminated falls back to word split) — consumers strip quotes themselves.
- **Import Merge Completeness Invariant (v0.336.0)**: when adding a new top-level declaration kind to `Ast`, it must be threaded through ALL of: `ImportResult`, `CachedImport`, the cache-hit copy loop, both child-tree rebuild sites (`import_resolver.zig` ×2), and `forward.zig`'s root merge — missing one makes declarations silently vanish after any import (composites were missed until v0.336.0). Semantic passes that rebuild tables must preserve every non-touched field (`template_ref` was dropped by autofk/suffix_inference, permanently disabling template_type_conflict).
- **Source-Rewriter Safety Contract (v0.336.0)**: `tune.zig` preserves everything before the first table header verbatim (preamble: `$`, `@version`, `~`), extracts only byte-identical field lines across ≥2 tables (conflicting definitions are never merged), orders template fields by first appearance (never hash iteration), and skips `^engine` headers. The formatter uppercases SQL keywords inside @if blocks but protects a line's first identifier whenever its second identifier is a Rune type symbol (`comment s128` stays lowercase; `sysdate created_on` still uppercases) and matches `@endif` by prefix + word boundary. `init` formats templates through `formatDialect` before writing so shipped files pass their own `format --check`.
...
| | `lint.zig` | Lint barrel — re-exports from `lint/rules.zig` (85 rules + `RuleInfo` + `RULE_INFO`), `lint/handlers/*.zig` (9 handler modules), `lint/format.zig` (text/JSON/SARIF), `lint/config.zig` (LintConfig, LintRule enum with `name()`, `description()`, `isFixable()`, TOML parsing), `lint/fix.zig` (auto-fix for 11 rules) |
| | `cli/lint_cmd.zig` | Lint CLI handler (extracted from main.zig) |
| | `cli/errors.zig` | CLI error handling (extracted from main.zig) |
