...
|-------|-------|-------|----------|
| Unit tests | 100 colocated `*_test.zig` files (wired via `tests.zig` comptime index) + inline tests in `diff/fields.zig`, `semantic/pass/*.zig` | ~1,548+ | Core logic + pipeline + colocated |
...
The lint module (`rune lint`) analyzes `.ss` schemas for quality issues. It runs after semantic analysis and produces diagnostic results. Supports 41 rules, 11 auto-fixable, with `--show-rules` and `--init` for discoverability.
