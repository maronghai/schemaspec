...
test "formatDiffSarif: produces valid SARIF structure" {
    ...
    const sarif = try formatDiffSarif(testing.allocator, d, .mysql);
    defer testing.allocator.free(sarif);
    try testing.expect(std.mem.indexOf(u8, sarif, "\"version\": \"2.1.0\"") != null);
    try testing.expect(std.mem.indexOf(u8, sarif, "\"ruleId\": \"schema/dropped-table\"") != null);
    try testing.expect(std.mem.indexOf(u8, sarif, "old_table") != null);
}
