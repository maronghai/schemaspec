...
test "isDatetimeSymType: t is datetime" {
    try std.testing.expect(type_map.isDatetimeSymType(.{ .simple = "t" }));
}

test "isDatetimeSymType: d is datetime" {
    try std.testing.expect(type_map.isDatetimeSymType(.{ .simple = "d" }));
}

test "isDatetimeSymType: n is not datetime" {
    try std.testing.expect(!type_map.isDatetimeSymType(.{ .simple = "n" }));
}

test "isNumericSymType: n is numeric" {
    try std.testing.expect(type_map.isNumericSymType(.{ .simple = "n" }));
}
...
test "isNumericSymType: s is not numeric" {
    try std.testing.expect(!type_map.isNumericSymType(.{ .simple = "s" }));
}

test "isNumericSymType: int_explicit is numeric" {
    try std.testing.expect(type_map.isNumericSymType(.{ .int_explicit = 11 }));
}

test "isNumericSymType: decimal_explicit is numeric" {
    try std.testing.expect(type_map.isNumericSymType(.{ .decimal_explicit = .{ .precision = 10, .scale = 2 } }));
}
