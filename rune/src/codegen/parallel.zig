...
/// Falls back to sequential compilation on thread spawn failure or individual table failure.
/// Returns the total bytes compiled.
fn compileGroupConcurrent(
    alloc: std.mem.Allocator,
    dialect: dialect_enum.Dialect,
    all_tables: []const typed_ast.TypedTable,
    group: []const usize,
    tables: *std.ArrayList(streaming.StreamingResult.TableOutput),
) !usize {
        ...
        for (results) |result_opt| {
            if (result_opt) |result| {
                ...
                tables.appendAssumeCapacity(.{
                    .name = all_tables[result.table_idx].name,
                    .sql = sql,
                    .line_no = all_tables[result.table_idx].line_no,
                });
                total_size += sql.len;
                result.arena.deinit();
            } else {
...
pub fn compileParallel(
    ...
    for (graph.groups) |group| {
        if (group.len == 1) {
            ...
            // All tables fit in one concurrent batch
            total_size += try compileGroupConcurrent(alloc, dialect, typed.tables, group, &tables);
        } else {
            // More tables than max_threads: compile in batches
            var i: usize = 0;
            while (i < group.len) {
                const batch_end = @min(i + max_threads, group.len);
                const batch_slice = group[i..batch_end];
                if (batch_slice.len == 1) {
                    const sql = try compileOneTable(alloc, dialect, typed.tables[batch_slice[0]]);
                    tables.appendAssumeCapacity(.{
                        .name = typed.tables[batch_slice[0]].name,
                        .sql = sql,
                        .line_no = typed.tables[batch_slice[0]].line_no,
                    });
                    total_size += sql.len;
                } else {
                    total_size += try compileGroupConcurrent(alloc, dialect, typed.tables, batch_slice, &tables);
                }
                i = batch_end;
            }
        }
