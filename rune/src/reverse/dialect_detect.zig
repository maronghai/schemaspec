...
pub fn detectSqlDialect(sql: []const u8) codegen.Dialect {
    ...
    if (std.mem.indexOf(u8, sql, "GENERATED ALWAYS AS IDENTITY") != null) scores[1] += 2; // shared with Db2
    ...
    if (std.mem.indexOf(u8, sql, "CHAR(16) FOR BIT DATA") != null) scores[5] += 3;
    if (std.mem.indexOf(u8, sql, "CHARACTER(16) FOR BIT DATA") != null) scores[5] += 3;
    if (std.mem.indexOf(u8, sql, "LONG VARCHAR") != null) scores[5] += 2;
    if (std.mem.indexOf(u8, sql, "NOT NULL WITH DEFAULT") != null) scores[5] += 2;
