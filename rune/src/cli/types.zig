...
pub const Command = union(enum) {
    ...
    init: struct { name: ?[]const u8, output: ?[]const u8, output_dir: ?[]const u8 = null },
...
pub const COMMAND_REGISTRY = [_]CommandInfo{
    ...
    .{ .name = "init", .args = "[name] [--output-dir <dir>]", .description = "Create a starter .ss schema file" },
...
pub const KNOWN_FLAGS = [_][]const u8{
    ...
    "--generators",     "--from-sql",    "--fix",         "--rules",        "--output-dir",
};
