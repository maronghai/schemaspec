...
test "classifyCheck: range [1, 100]" {
    try std.testing.expectEqual(CheckKind.range, classifyCheck("1, 100", '[', ']'));
}

test "classifyCheck: range_upper_exclusive [1, 100)" {
    try std.testing.expectEqual(CheckKind.range_upper_exclusive, classifyCheck("1, 100", '[', ')'));
}

test "classifyCheck: range_lower_exclusive (1, 100]" {
    try std.testing.expectEqual(CheckKind.range_lower_exclusive, classifyCheck("1, 100", '(', ']'));
}

test "classifyCheck: range_both_exclusive (1, 100)" {
    try std.testing.expectEqual(CheckKind.range_both_exclusive, classifyCheck("1, 100", '(', ')'));
}

test "classifyCheck: in_list {active, inactive}" {
    try std.testing.expectEqual(CheckKind.in_list, classifyCheck("active, inactive", '{', '}'));
}

test "classifyCheck: comparison {price > 0}" {
    try std.testing.expectEqual(CheckKind.comparison, classifyCheck("price > 0", '{', '}'));
}

test "classifyCheck: comparison with AND" {
    try std.testing.expectEqual(CheckKind.comparison, classifyCheck("price > 0 AND price < 10000", '[', ']'));
}

test "classifyCheck: single value → comparison" {
    try std.testing.expectEqual(CheckKind.comparison, classifyCheck("active", '{', '}'));
}
