...
fn freeFk(alloc: std.mem.Allocator, fk: FkDecl) void {
    for (fk.fields) |f| alloc.free(f);
    alloc.free(fk.fields);
    alloc.free(fk.ref_table);
    for (fk.ref_fields) |f| alloc.free(f);
...
test "parseFk: compound ultra — > table.field1.field2" {
    const alloc = std.testing.allocator;
    const tokens = [_][]const u8{ ">", "projects.org_id.project_id" };
    const fk = try pf.parseFk(alloc, makeLine(&tokens, 1));
    defer freeFk(alloc, fk);
    try std.testing.expectEqualStrings("projects", fk.ref_table);
    try std.testing.expectEqual(@as(usize, 2), fk.ref_fields.len);
    try std.testing.expectEqualStrings("org_id", fk.ref_fields[0]);
    try std.testing.expectEqualStrings("project_id", fk.ref_fields[1]);
    // Ultra shorthand infers local field from ref_table: projects_id
    try std.testing.expectEqual(@as(usize, 1), fk.fields.len);
    try std.testing.expectEqualStrings("projects_id", fk.fields[0]);
}

test "parseFk: single local field with compound ref — user_id projects.org_id.project_id" {
    const alloc = std.testing.allocator;
    const tokens = [_][]const u8{ "user_id", "projects.org_id.project_id" };
    const fk = try pf.parseFk(alloc, makeLine(&tokens, 1));
    defer freeFk(alloc, fk);
    try std.testing.expectEqual(@as(usize, 1), fk.fields.len);
    try std.testing.expectEqualStrings("user_id", fk.fields[0]);
