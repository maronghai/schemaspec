...
test "Oracle GENERATED BY DEFAULT AS IDENTITY" {
    ...
    const sql =
        ...
        \\);
    ;
    var parser = try SqlParser.init(alloc, sql, .oracle);
    const result = try parser.parse();
    defer freeResult(alloc, &parser, result);
    ...
    try std.testing.expectEqual(@as(usize, 2), tbl.columns.len);
    try std.testing.expectEqualStrings("id", tbl.columns[0].name);
    try std.testing.expectEqualStrings("NUMBER(10)", tbl.columns[0].type_sql);
    try std.testing.expect(tbl.columns[0].auto_increment);
    try std.testing.expect(tbl.columns[0].primary_key);
    try std.testing.expectEqualStrings("name", tbl.columns[1].name);
    try std.testing.expectEqualStrings("VARCHAR2(100)", tbl.columns[1].type_sql);
}

test "Oracle GENERATED BY DEFAULT ON NULL AS IDENTITY" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
    ...
    const col = result.schema.tables[0].columns[0];
    try std.testing.expectEqualStrings("NUMBER(10)", col.type_sql);
    try std.testing.expect(col.auto_increment);
    try std.testing.expect(col.primary_key);
}

test "Oracle GENERATED BY DEFAULT AS IDENTITY with options" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
...
test "Oracle GENERATED ALWAYS AS IDENTITY with CACHE" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  id NUMBER(10) GENERATED ALWAYS AS IDENTITY START WITH 100 INCREMENT BY 10 CACHE 20
        \\);
    ;
    var parser = try SqlParser.init(alloc, sql, .oracle);
    const result = try parser.parse();
...
test "Oracle NUMBER(P,S) types" {
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("NUMBER(10,2)", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("NUMBER(8,2)", tbl.columns[1].type_sql);
}

test "Oracle VARCHAR2 and NVARCHAR2 types" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  name VARCHAR2(100) NOT NULL,
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("VARCHAR2(100)", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("NVARCHAR2(500)", tbl.columns[1].type_sql);
}

test "Oracle TIMESTAMP WITH TIME ZONE" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        \\  updated_at TIMESTAMP WITH TIME ZONE,
        \\  ts_local TIMESTAMP WITH LOCAL TIME ZONE
        \\);
    ;
    var parser = try SqlParser.init(alloc, sql, .oracle);
    const result = try parser.parse();
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("TIMESTAMP", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("TIMESTAMP WITH TIME ZONE", tbl.columns[1].type_sql);
    try std.testing.expectEqualStrings("TIMESTAMP WITH LOCAL TIME ZONE", tbl.columns[2].type_sql);
}

// ─── Db2-specific tests ────────────────────────────────────────────

test "Db2 GENERATED ALWAYS AS IDENTITY" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        ...
        \\);
    ;
    var parser = try SqlParser.init(alloc, sql, .db2);
    const result = try parser.parse();
    defer freeResult(alloc, &parser, result);
    ...
    try std.testing.expect(tbl.columns[0].primary_key);
    try std.testing.expectEqualStrings("name", tbl.columns[1].name);
    try std.testing.expectEqualStrings("VARCHAR(100)", tbl.columns[1].type_sql);
}

test "Db2 GENERATED ALWAYS AS IDENTITY with options" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
...
test "Db2 BOOLEAN type" {
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("BOOLEAN", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("BOOLEAN", tbl.columns[1].type_sql);
}

test "Db2 DECIMAL and NUMERIC types" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  amount DECIMAL(10,2),
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("DECIMAL(10,2)", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("NUMERIC(8,2)", tbl.columns[1].type_sql);
}

test "Db2 TIMESTAMP WITH TIME ZONE" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
// ─── Oracle identity and block comment tests ────────────────────

test "Oracle identity START WITH INCREMENT BY" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  id NUMBER(10) GENERATED BY DEFAULT AS IDENTITY START WITH 100 INCREMENT BY 5
        \\);
    ;
    var parser = try SqlParser.init(alloc, sql, .oracle);
    const result = try parser.parse();
...
test "Oracle TIMESTAMP WITH LOCAL TIME ZONE" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("TIMESTAMP", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("TIMESTAMP WITH LOCAL TIME ZONE", tbl.columns[1].type_sql);
}

// ─── Db2 type tests ────────────────────────────────────────────

test "Db2 BIGINT and SMALLINT" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
        \\  big_col BIGINT DEFAULT 0,
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("BIGINT", tbl.columns[0].type_sql);
    try std.testing.expectEqualStrings("SMALLINT", tbl.columns[1].type_sql);
}

test "Db2 DOUBLE PRECISION" {
    const alloc = std.testing.allocator;
    const sql =
        \\CREATE TABLE t (
    ...
    const tbl = result.schema.tables[0];
    try std.testing.expectEqualStrings("DOUBLE PRECISION", tbl.columns[0].type_sql);
}
