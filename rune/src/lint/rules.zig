...
const RULES = [_]RuleEntry{
    ...
    .{ .rule = .cross_dialect_types, .handler = checkCrossDialectTypes },
};
...
fn checkCrossDialectTypes(alloc: std.mem.Allocator, results: *std.ArrayList(LintResult), ast: ResolvedAst, _: LintConfig) !void {
    for (ast.tables) |table| {
        for (table.fields) |field| {
            switch (field.type_info) {
                .simple => |s| {
