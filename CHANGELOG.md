...
### Fixed
- **`lint --fix` no longer corrupts schemas** (destructive no-op on the main migration path): the default-value fixers appended `" = 0"` / `" = false"` / `" = null"` — but the tokenizer splits fields on whitespace, so the standalone `=` and value tokens were warned about and silently dropped. Every fixed file was rewritten, every inserted default vanished from the SQL output, and the exit code stayed 0. Fix output now uses the spec's attached form (` =0`, ` =false`, ` =null`, ` =CURRENT_TIMESTAMP`, ` ='{}'`); `rune migrate` (which auto-lints by default) diffs against an intact schema instead of a corrupted one.
- **`no-timestamps` fixer no longer fuses table headers into field lines**: both timestamp insertion points (inter-table flush and EOF) omitted the trailing newline, producing `updated_at t# next_table` in multi-table schemas — the following header was swallowed as field text. Both now terminate with `\n`.
- **Diff/migrate sees inline column-level FKs**: inline FKs live on `Field.fk` while codegen merges them with table-level FKs in `TypeResolver.resolve()` — but the diff engine compared raw `ResolvedTable.fks`, so adding or removing `user_id > users.id` produced an empty migration while the compiled SQL gained/lost a constraint. `diffTable`, rename-path, and create-table paths now diff over a merged view (`collectAllFks`) matching codegen's ordering.
- **Compound foreign keys keep all reference columns**: `> user_id org_id users.id users.org` parsed only the first dot-token and silently dropped the rest, emitting `FOREIGN KEY (user_id, org_id) REFERENCES users(id)` — mismatched column counts that every backend rejects at execution time. Consecutive same-table dot-tokens now extend `ref_fields`; actions parse after the last ref token; a column-count mismatch between local and reference sides is a hard error (multi-token compound form only — the single-token dotted projection form is unchanged). Prisma `@relation` lists all fields/references instead of `[fields[0]]`.
- **Rename detection consumes its targets**: two dropped fields with the same signature could both rename onto one added field, generating `CHANGE COLUMN a z` + `CHANGE COLUMN b z` — a colliding pair that cannot execute. The second match now degrades to drop+add.
- **Incremental cache hash covers SqlType payloads**: `computeTableHash` hashed only `@tagName(col.sql_type)`, so `varchar(50)` vs `varchar(255)` and differing decimal precision/scale collided to the same key and served stale SQL under `--cache`. varchar length, decimal precision/scale, and raw_sql/passthrough payloads now enter the hash.
- **knex/drizzle/prisma preserve decimal precision/scale**: all three emitted bare `decimal`/`Decimal`, degrading migrated columns to driver-default precision. knex → `table.decimal('amount', 3, 2)`, drizzle → `decimal('amount', { precision: 3, scale: 2 })`, prisma → `Decimal @db.Decimal(3, 2)`. typeorm/sqlalchemy/openapi/json-schema were already correct.
- **Quoted string defaults render correctly**: `name s32 ='x'` produced `DEFAULT ''x''` (double-wrapped quotes) because emitDefault re-quoted an already-quoted literal; it now strips outer quotes like the CHECK in-list path (`='x'` → `'x'`, `=''` → `''`). Boolean/null keyword defaults are matched case-insensitively and `TRUE`/`FALSE` added to the bare-keyword set — `flag b =false` previously emitted `DEFAULT 'false'` (a string comparison that can never be true).

### Added
- Golden tests: tests/compound-fk.* (mysql/pg/sqlite/prisma — compound FK round-trip). Unit tests 2094 → 2100 (+rename-consumption, inline-FK-signature, hash-payload, prisma/drizzle decimal, prisma-compound-relation cases; knex decimal golden updated for precision args).
- Known issue recorded: tests/test_migrate_status.sh fails 5/7 on Windows-native binaries (null-byte output through bash pipes) — pre-existing since before v0.332.0, unrelated to this release.
...
- **Source formatting**: Applied `zig fmt` to all source files (12 lint handler modules and 7 test files formatted)
- **Test artifact cleanup**: Removed temporary `*.lint-fixed.ss` files generated by `rune lint --fix` during test runs
- **Documentation metrics verified**: 69,906 total lines (45,218 production + 24,688 tests) across 327 `.zig` files; 2,022 unit tests; 84 lint rules; 111 `REVERSE_MAP` entries

### Test Infrastructure
- **Full test suite validation**: All 2,022 unit tests pass (`zig build test`)
- **Golden test suites**: All 33 golden test suites pass (`bash tests/test_coverage.sh`)
- **Benchmark regression check**: No regressions beyond 10% threshold (`zig build bench -- --check`)

### Changed
- Version synchronized to 0.308.0 across `VERSION`, `rune/VERSION`, `rune/build.zig.zon`
- ROADMAP.md current version header updated to 0.308.0
- v0.308.0 entry added to ROADMAP.md Release History
